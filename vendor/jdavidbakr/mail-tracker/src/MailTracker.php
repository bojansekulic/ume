...
//use Vanguard\Http\Controllers\Web\LaunchCampaignController;
use Auth;
use Vanguard\Http\Controllers\Web\LaunchCampaignController;
...
class MailTracker extends LaunchCampaignController implements \Swift_Events_SendListener {
...
//    	$tracking_pixel = '<img border=1 width=50 alt="" height=50 src="'.url('tracker.php?hash='.$hash).'" />';
...
//                    'content'=> config('mail-tracker.log-content', true) ? (strlen($original_content) > 65535 ? substr($original_content, 0, 65532) . "..." : $original_content) : null,
                ...
                $kampanje= SentEmail::all();

                $pivot = Campaign::find($campaignId);
                $pivot->sent_emails()->sync($kampanje);
            }
        }
    }
