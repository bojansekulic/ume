...
use jdavidbakr\MailTracker\Events\SubmitedEmailEvent;
...
class MailTrackerController extends Controller
{
    ...
    public function getL($url, $hash)
    {
        ...
        // hvatanje hasha u link

        return redirect($url.'?hash='.$tracker->hash);
    }
    ...
    public function submited(Request $request)
    {
        ...
        $counter = Model\SentEmail::where('hash', $request->hash)
            ->first();
        ...
        if ($counter) {
            ...
            Event::fire(new SubmitedEmailEvent($counter));
        }

        return view('submit_catch.submited');
